...
function overwritePrivacyManifest (manifest, locations) {
    ...
    const defaultPrivacyManifest = path.join(__dirname, '..', 'templates', 'project', 'CApp', 'PrivacyInfo.xcprivacy');
...
function updateProject (platformConfig, locations) {
    const plistFile = path.join(locations.xcodeCordovaProj, 'CApp-Info.plist');
...
function handleOrientationSettings (platformConfig, project) {
    function setProp (name, value) {
        if (value) {
            project.xcode.updateBuildProperty(`INFOPLIST_KEY_${name}`, `"${value}"`, null, 'CApp');
        } else {
            project.xcode.updateBuildProperty(`INFOPLIST_KEY_${name}`, null, null, 'CApp');
        }
    }
...
function handleBuildSettings (platformConfig, locations, infoPlist) {
    ...
    project.xcode.updateBuildProperty('PRODUCT_BUNDLE_IDENTIFIER', `"${pkg}"`, null, 'CApp');

    events.emit('verbose', `Set PRODUCT_NAME to ${name}.`);
    project.xcode.updateBuildProperty('PRODUCT_NAME', `"${name}"`, null, 'CApp');

    events.emit('verbose', `Set MARKETING_VERSION to ${version}.`);
    project.xcode.updateBuildProperty('MARKETING_VERSION', version, null, 'CApp');

    events.emit('verbose', `Set CURRENT_PROJECT_VERSION to ${CFBundleVersion}.`);
    project.xcode.updateBuildProperty('CURRENT_PROJECT_VERSION', CFBundleVersion, null, 'CApp');

    if (displayName !== name) {
        events.emit('verbose', `Set INFOPLIST_KEY_CFBundleDisplayName to ${displayName}.`);
        project.xcode.updateBuildProperty('INFOPLIST_KEY_CFBundleDisplayName', `"${displayName}"`, null, 'CApp');
    } else {
        project.xcode.updateBuildProperty('INFOPLIST_KEY_CFBundleDisplayName', '"$(PRODUCT_NAME)"', null, 'CApp');
    }
