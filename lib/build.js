...
function createProjectObject (projectPath) {
    const locations = {
        root: projectPath,
        pbxproj: path.join(projectPath, 'CApp.xcodeproj', 'project.pbxproj')
    };
...
module.exports.run = function (buildOpts) {
    ...
    return Promise.resolve()
        ...
        .then(() => {
            ...
            events.emit('log', `Building project: ${path.join(projectPath, 'CApp.xcworkspace')}`);
...
function getXcodeBuildArgs (projectPath, configuration, emulatorTarget, buildConfig = {}) {
    ...
    if (buildConfig.device && !buildConfig.catalyst) {
        options = [
            '-workspace', customArgs.workspace || 'CApp.xcworkspace',
            '-scheme', customArgs.scheme || 'CApp',
            '-configuration', customArgs.configuration || configuration,
            '-destination', customArgs.destination || 'generic/platform=iOS',
            '-archivePath', customArgs.archivePath || 'CApp.xcarchive'
        ];
