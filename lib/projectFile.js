...
function parseProjectFile (locations) {
    ...
    const config_file = path.join(project_dir, 'CApp', 'config.xml');
    ...
    const xcode_dir = path.join(project_dir, 'CApp');
    ...
    cachedProjectFiles[project_dir] = {
        ...
        getPackageName: function () {
            return xcodeproj.getBuildProperty('PRODUCT_BUNDLE_IDENTIFIER', undefined, 'CApp').replace(/^"/, '').replace(/"$/, '');
        },
